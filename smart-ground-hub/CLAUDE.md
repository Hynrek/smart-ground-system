...
- **`Serie`** (formerly Ablauf): A shooting sequence. `ownership`: `user` (private) or `range` (visible to all on that range). Steps stored as JSON array. `published`: `false` by default; range-owned Serien are hidden from regular users until an admin sets `published = true` via `PATCH /api/serien/{id}/published`. Serie `PUT` (`UpdateSerieRequest`) accepts an optional `steps` array → in-place step edit keeping the **stable Serie ID**; this is why a Passe can safely reference Serien by ID.
  - **Sync (#2):** `Serie` carries an indexed `updated_at` (`@PreUpdate`-stamped) and a `deleted` flag; `deleteSerie` is a **soft-delete**. A class-level `@SQLRestriction("deleted = false")` hides tombstones from every managed read (list/get, Passe join, PlayInstance, Competition) — identical to the old hard-delete. `GET /api/sync/serien` (`SyncController`/`SerieSyncService`, `SerieRepository.findForSyncFrom` — a **native** query that deliberately bypasses the restriction) serves a cursor page **including** tombstones so a Node can mirror deletions. `GET /api/sync/**` is permitted unauthenticated for now (temporary — service-token auth is #6), matching the OTA-download GET posture.
